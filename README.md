...
## 기능 목록(커밋 기준)
- [ ]입력
  - [ ] @woowacourse/mission-utils에서 제공하는 Console API를 사용하여 입력을 받는다.
  - [ ] 문자열의 좌우 공백을 없앤다.
- [ ]파싱
  - [ ] 구분자 식별
    - `기본 구분자`(`,`, `:`)
...
- [ ]검증 & 예외처리
  - [ ] 숫자들이 대해 0이나 양의 정수인지 검증한다. (다른 문자, 음수, 소수 x)
  - [ ] 구분자의 공백이 아닌 길이가 1인 하나의 문자인지 검증한다.
  - [ ] 구분자가 숫자가 아님을 검증한다.
  - [ ] 커스텀 구분자가 하나임을 검증한다.(여러 번 지정 불가능)
- [ ]연산(덧셈)
  - [ ] 숫자들을 모두 더한다.
- [ ]출력
